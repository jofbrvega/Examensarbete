...
<header class="z-50 w-full">
    <nav x-data="{isOpen: false}" class="flex items-center justify-between">
        ...
        <?php endif; ?>
        <button type="button" aria-label="toggle-menu" @click="isOpen = !isOpen" class="relative z-30 flex flex-col justify-between h-12 focus:outline-none top-1 open">
            <img class="flex h-12" src="<?= esc_url(get_template_directory_uri()); ?>/assets/images/icon-hamburger-menu.png" alt="hamburger-menu">
        </button>
